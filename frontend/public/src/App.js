  ...
  const profile = {
    ...
    projects: [
      ...
      },
      {
        ...
        hasLink: true,
        // REPLACE THE URL BELOW WITH YOUR ACTUAL LINK IF IT IS DIFFERENT
        ...
        <section style={{ background: '#1e293b', padding: '25px', borderRadius: '12px', marginBottom: '25px', border: '1px solid #334155' }}>
          ...
          {profile.projects.map((proj, index) => (
            <div key={index} style={{ marginTop: '20px', borderBottom: index !== profile.projects.length - 1 ? '1px dashed #334155' : 'none', paddingBottom: '15px' }}>
              ...
              {/* Renders a live blue link button if the project has a URL */}
              {proj.hasLink && (
                <a 
                  href={proj.url} 
                  target="_blank" 
                  rel="noopener noreferrer" 
                  style={{ display: 'inline-block', backgroundColor: '#38bdf8', color: '#0f172a', padding: '8px 16px', borderRadius: '6px', fontWeight: 'bold', textDecoration: 'none', fontSize: '0.85rem', marginTop: '5px', transition: 'background 0.2s' }}
                >
                  🚀 View Live Project
                </a>
              )}
